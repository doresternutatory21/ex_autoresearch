...
defmodule ExAutoresearch.Experiments.V_VERSION_ID do
  @moduledoc "DESCRIPTION OF WHAT THIS VERSION CHANGES" do
  # @moduledoc "Try embedding dimension 24 with the successful vocab_size=2 tanh-based MLP-only 2x architecture (v_fbvl0t0). Since we've reached the minimum possible vocabulary size of 2, the next optimization is to reduce embedding dimension. Previous attempts at reducing embedding dimension failed with larger vocabularies, but with vocab_size=2 providing such minimal parameter count, reducing n_embd from 32 to 24 might further optimize the model without losing representational capacity. This maintains all other optimal elements: 1-layer, MLP-only, 2x multiplier, tanh activation, post-norm, learning rate 0.0075, and batch size 16."
  ...
  defp transformer_block(x, config, i) do
    # Skip attention entirely, just do MLP with residual and post-norm
    mlp =
      x
      |> Axon.dense(2 * config.n_embd, use_bias: false, name: "b#{i}_mlp_up")
      |> Axon.activation(:tanh)
      |> Axon.dense(config.n_embd, use_bias: false, name: "b#{i}_mlp_down")

    Axon.add(x, mlp) |> Axon.layer_norm(name: "b#{i}_norm", epsilon: 1.0e-6)
  end
  ...
  def optimizer do
    Polaris.Optimizers.adamw(learning_rate: 0.0075)
  end
end
